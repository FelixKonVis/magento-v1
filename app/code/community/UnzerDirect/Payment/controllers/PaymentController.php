<?php
class UnzerDirect_Payment_PaymentController extends Mage_Core_Controller_Front_Action
{
    /**
     * Get payment method
     *
     * @return UnzerDirect_Payment_Model_Payment
     */
    public function getPayment()
    {
        return Mage::getSingleton('unzerdirect_payment/payment');
    }

    /**
     * Handle redirect to UnzerDirect
     */
    ...
    public function cancelAction()
    {
        ...
        $session = $this->_getSession();
        $session->setQuoteId($session->getUnzerDirectQuoteId(true));
        if ($session->getLastRealOrderId()) {
            ...
            Mage::helper('unzerdirect_payment/checkout')->restoreQuote();
        }
    ...
    /**
     * Handle customer being redirected from UnzerDirect
     */
            ...
            if ($request->accepted && $operation->type == 'authorize') {
                ...
                if ((int)$payment->getConfigData('transactionfee') == 1) {
                    ...
                    if ((int)$payment->getConfigData('specifytransactionfee') == 1) {
                        $fee_text = " " . Mage::helper('unzerdirect_payment')->__("inkl. %s %s i transaktionsgebyr", $fee, $order->getData('order_currency_code'));
                    }
                ...
                Mage::helper('unzerdirect_payment')->createTransaction($order, $request->id, Mage_Sales_Model_Order_Payment_Transaction::TYPE_AUTH);
                 ...
                 * We wait until this moment since qpCancel will fail without a row in unzerdirect_payment_order_status
                 */
                ...
                if ((int)Mage::getStoreConfig('cataloginventory/item_options/manage_stock') == 1) {
                    Mage::helper('unzerdirect_payment')->removeFromStock($order->getIncrementId());
                }
            }
        } else {
    ...
    public function updateOrderByCallback($order, $data){
        ...
        if($order->getBillingAddress()) {
            $countryCode = Mage::helper('unzerdirect_payment')->convertCountryAlphas3To2($billingAddress->country_code);
        ...
        if($order->getShippingAddress()) {
            $countryCode = Mage::helper('unzerdirect_payment')->convertCountryAlphas3To2($shippingAddress->country_code);
