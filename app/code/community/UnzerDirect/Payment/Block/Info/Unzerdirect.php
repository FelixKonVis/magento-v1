...
class UnzerDirect_Payment_Block_Info_Unzerdirect extends Mage_Payment_Block_Info
{
    ...
    {
        parent::_construct();
        $this->setTemplate('unzerdirectpayment/info/default.phtml');
    }

    /**
     * Get UnzerDirect info HTML
     *
     * @return string
     */
    public function getUnzerDirectInfoHtml()
    {
        $res = "";
            ...
            if (is_array($row)) {
                if ($row['qpstat'] == 20000) {
                    ...
                    if ($row['qpstat'] == 20000) {
                        ...
                        $cardÍmagePath = sprintf('images/unzerdirectpayment/cards/%s.png', $row['cardtype']);
