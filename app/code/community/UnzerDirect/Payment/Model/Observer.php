<?php
class UnzerDirect_Payment_Model_Observer
{
    public function autoRegisterState(Varien_Event_Observer $observer)
    {
        ...
        if (isset($data['unzerdirect_state'])) {
            Mage::getSingleton('core/session')->setUnzerDirectState($data['unzerdirect_state']); // Branding
        }
    ...
    */
    public function placeOrder($observer)
    {
        try {
            ...
            if ($payment instanceof UnzerDirect_Payment_Model_Method_Abstract) {
    ...
    public function saveOrder($observer)
    {
        ...
        try {
            ...
            $session->addException($e, Mage::helper('unzerdirect_payment')->__("Can't change status of order", $e->getMessage()));
        }
    ...
    public function onBlockHtmlBefore(Varien_Event_Observer $observer)
    {
        ...
        if ($block->getType() === 'adminhtml/sales_order_grid') {
            ...
            $block->addColumnAfter('fraudprobability', array(
                ...
                'width'    => '40px',
                'weight'   => '100',
                'renderer' => 'UnzerDirect_Payment_Model_Sales_Order_Grid_Fraudprobability',
            ), 'status');
            ...
            $massAction->addItem('unzerdirect_bulk_capture', array(
                'label' => 'Capture with UnzerDirect',
                'url' => $block->getUrl('adminhtml/unzerdirect/massCapture')
            ));
        }
    }
    ...
    public function onCheckoutSubmitAllAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($order->getPayment()->getMethodInstance() instanceof UnzerDirect_Payment_Model_Method_Abstract) {
            $payment = Mage::getSingleton('unzerdirect_payment/payment');
    ...
    {
        return $store->getUrl('unzerdirect/payment/linksuccess');
    }
    ...
    {
        return $store->getUrl('unzerdirect/payment/cancel');
    }
    ...
    {
        return $store->getUrl('unzerdirect/payment/callback');
    }
}
