...
class UnzerDirect_Payment_Model_Type_Mobilepay
{
    /**
    ...
    {
        if ($quote->isVirtual()) {
            $quote->getBillingAddress()->setPaymentMethod('unzerdirect_mobilepay');
        } else {
            $quote->getShippingAddress()->setPaymentMethod('unzerdirect_mobilepay');
        }

        $data = array();
        $data['method'] = 'unzerdirect_mobilepay';
    ...
    public function saveShippingMethod(Mage_Sales_Model_Quote $quote, $shippingMethod = null)
    {
        if (is_null($shippingMethod)) {
            $shippingMethod = Mage::getStoreConfig('payment/unzerdirect_mobilepay/default_shipping_method');
        }
