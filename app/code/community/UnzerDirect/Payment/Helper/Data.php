...
class UnzerDirect_Payment_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function qpCreatePayment(Mage_Sales_Model_Order $order)
    {
        ...
        if ($textOnStatement = Mage::getStoreConfig('payment/unzerdirect_payment/text_on_statement')) {
    ...
    public function qpCreateMobilepayPayment(Mage_Sales_Model_Order $order, $shippingData)
    {
        $resource = Mage::getSingleton('core/resource');
        $connection = $resource->getConnection('core_write');
        $table = $resource->getTableName('unzerdirect_order_status');
        $connection->insert($table, array('ordernum' => $order->getIncrementId()));
    ...
    public function capture($payment, $amount)
    {
        ...
        $unzerDirectTransactionId = $qpOrderStatus['transaction'];
        ...
        if ((int)($amount * 100) <= ((int)$qpOrderStatus['amount'] - (int)$capturedAmount)) {
            ...
            try {
                $result = $this->qpCapture($unzerDirectTransactionId, round($amount * 100));
            } catch (Exception $e) {
            ...
            if ($result->qp_status_code == "20000") {
                $session->addSuccess(Mage::helper('unzerdirect_payment')->__('Betalingen for ordre %s er hævet online.', $order->getIncrementId()));
                ...
                $this->createTransaction($order, $unzerDirectTransactionId, Mage_Sales_Model_Order_Payment_Transaction::TYPE_CAPTURE);
            } else {
                Mage::throwException("UnzerDirect Response: " . $result->qp_status_msg);
            }
        } else {
            Mage::throwException(Mage::helper('unzerdirect_payment')->__('Der forsøges at hæve et højere beløb en tilladt'));
        }
    ...
    public function refund(Mage_Sales_Model_Order $order, $refundtotal)
    {
        ...
        if (($refundtotal * 100) <= $qpOrderStatus['capturedAmount']) {
            ...
            if ($result->qp_status_code == "20000") {
                $session->addSuccess(Mage::helper('unzerdirect_payment')->__('Kreditnota refunderet online'));
            ...
            Mage::throwException(Mage::helper('unzerdirect_payment')->__('Max beløb der kan refunderes: %s', $qpOrderStatus['capturedAmount']));
        }

        $order->addStatusToHistory($order->getStatus(), Mage::helper('unzerdirect_payment')->__('Kreditnota refunderede %s online', number_format($refundtotal, 2, ",", "")), false);
        $order->save();
    }
    ...
    public function cancel(Mage_Sales_Model_Order $order)
    {
        ...
        $this->apiKey = Mage::getStoreConfig('payment/unzerdirect_payment/apikey', $order->getStoreId());
        ...
        if ($result->qp_status_code == "20000") {
            $session->addSuccess(Mage::helper('unzerdirect_payment')->__('Betalingen blev annulleret online'));
            $write = Mage::getSingleton('core/resource')->getConnection('core_write');
        ...
        $order->addStatusToHistory($order->getStatus(), Mage::helper('unzerdirect_payment')->__('Betalingen blev annulleret online'), false);
        $order->save();
    }
    ...
    /**
     * Get row from unzerdirect_payment_order_status table
     *
     * @param $order_id
     * @return bool
     */
    public function getUnzerDirect($order_id)
    {
        if ($order_id) {
    ...
    {
        $data = $this->getUnzerDirect($order_id);
        if ($data) {
    ...
    {
        $pay = $this->getUnzerDirect($order_id);
        if ($pay) {
            if (!$pay['ordernum']) {
                return 'no_cart';
            } else {
    ...
    public function getImage($order_id)
    {
        ...
        if ($order_id) {
            $order = Mage::getModel('sales/order')->load($order_id);
            if ($order) {
                $payment = $this->getUnzerDirect($order_id);
    ...
    {
        return (string) Mage::getConfig()->getNode()->modules->UnzerDirect_Payment->version;
    }
